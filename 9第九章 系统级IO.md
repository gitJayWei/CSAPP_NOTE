...
- Linux系统中，内核通过三个数据结构来表示打开的文件，以此实现文件共享；

##### 9.8.1. 三个数据结构

- 描述符表(descriptor table)
  - 每个进程都有独立的描述符表，表中的表项通过进程打开的文件描述符来索引；
  - 每个打开的描述符表项 指向文件表中的一个表项；
- 文件表(file table)
  - 用于表示系统中打开的文件的集合，所有进程共享这张表；
  - 表项组成：
    - 当前文件位置：记录文件读写操作的当前位置；
    - 引用计数(reference count)：表示当前指向该表项的描述符表项数；例如，有多个描述符指向同一个文件表表项时，引用计数就会相应增加；关闭一个描述符时，会减少对应文件表表项中的引用计数；只有当引用计数为零，内核才会删除这个文件表表项；
    - 指向 v - node 表的指针：通过这个指针可以关联到 v - node 表中对应的表项，获取文件的更多元数据；
- v - node 表(v - node table)
  - 所有进程共享；
  - 每个表项包含`stat`结构中的大多数信息，例如文件访问权限（`st_mode` ）、文件大小（`st_size` ）等，用于存储文件的详细属性；
